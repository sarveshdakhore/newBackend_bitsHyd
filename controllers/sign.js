...
export const check_jwt = async (req, res) => {
    ...
    res.status(200).json({ message: "JWT verified.", data: { name: user.name, email: user.email, password: password, jwt: newJwt } });
};
...
export const hero_register = async (req, res) => {
    try {
        ...
        const tokenLink = generateTokenLink(payload, expTime, base_url, route, SECRET_KEY)+"/"+encodedEmail;
