...
async function main() {
  ...
  formFields.push(await prisma.formField.create({
    data: {
  ...
  const formTitle = "FORM XIV - APPLICATION FOR REGISTRATION OF COPYRIGHT [SEE RULE 70]";
  ...
  const embeddingVector = await generateEmbeddings(`${formTitle} ${formDescription}`);
  ...
  await prisma.formValue.create({
    data: {
      ...
      value: 'A comprehensive software solution for data analytics using AI and ML algorithms'
    }
  });
